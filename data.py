...
def load_fashion_mnist_data(dir_path):
    ...
    testloader = torch.utils.data.DataLoader(testset, batch_size=1000, shuffle=True, generator=torch.Generator(device='cuda'))
    return trainloader, testloader
...
def save_test_data(config):
    ...
    if config['data'] == 'cifar10':
        ...
        transform = transforms.Compose([transforms.Resize(32), transforms.ToTensor(),
                                        transforms.Normalize((0.2860,), (0.3530,))])
        testset = datasets.FashionMNIST(dir_path + '/data', download=True, train=False, transform=transform)
    testloader = torch.utils.data.DataLoader(testset, batch_size=1, shuffle=False, generator=torch.Generator(device='cuda'))
    test_data = []
    label_num = [0] * 10
    ...
    for batch_idx, (inputs, targets) in enumerate(testloader):
        ...
        label_idx = int(targets[0].item())
        if label_idx < config['t1'] and label_num[label_idx] >= 1000:
            continue
        if label_idx >= config['t1'] and label_num[label_idx] >= 20:
            continue
